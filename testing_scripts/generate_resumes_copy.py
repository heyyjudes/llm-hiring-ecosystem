...
def create_chat_gpt_resumes(marked_df, job_name: str, job_description: str, verbose: bool = False, marked_only: bool = True, original_column_name: str = 'CV') -> str:
    if marked_only:
    ...
    if original_column_name !="CV":
        ...
        column_name = f"gt4o-Improved {job_name} CV"
    marked_df[column_name] = pd.NA

    print('prepared all', flush=True)
    # Generate tailored resumes on only the entries marked for experiments
    generate = lambda resume : constants.gpt4o_conversation(get_prompt_messages(original_cv= resume, job_description=job_description, job_name = job_name))
    #tailor_resume(input_resume = resume, job_description = job_description, model_name = model_name, verbose = verbose)
...
def create_modified_resumes(marked_df, model_name: str, job_name: str, job_description: str, verbose: bool = False, marked_only: bool = True, original_column_name: str = 'CV') -> str:
    ...
    if original_column_name !="CV":
        ...
        column_name = f"{model_name}-Improved {job_name} CV"
    marked_df[column_name] = pd.NA
...
if __name__ == "__main__":
    ...
    create_chat_gpt_resumes(marked_df=marked_ui_df, job_name=constants.BITS_ORCHESTRA_PM_JOB_NAME, job_description=constants.BITS_ORCHESTRA_PM_JOB_DESCRIPTION)
