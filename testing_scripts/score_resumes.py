...
def append_scores(dataframe_labeled, job_description, job_title, modified_or_not):
    #appends score of similarity of Job title to dataframe.
    scores = [0 for i in range(len(dataframe_labeled))]
    counter=0
    print("enter function", flush=True)
    cv_label = 'CV'
    column_title = job_title+" Score"
    if modified_or_not == True:
...
if __name__ == "__main__":
    ...
    """
    append_scores(labeled_df, job_doordash_pm, "DoorDash PM", False)
    labeled_df.to_csv("data/Scored_Resumes.csv")